...
class MyLinearRegression():
    ...
    def __init__(self, thetas, alpha=0.001, n_cycle=1000):
        ...
        self.thetas = np.array(thetas).flatten().astype("float64")
    ...
    def fit_(self, x, y):
        ...
        # new_theta = self.thetas.astype("float64")
        for _ in range(self.max_iter):
            nabla = x_padded.T.dot(x_padded.dot(self.thetas) - y) / y.shape[0]
            self.thetas[0] = self.thetas[0] - self.alpha * nabla[0]
            self.thetas[1] = self.thetas[1] - self.alpha * nabla[1]
